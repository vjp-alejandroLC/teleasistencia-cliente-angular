...
export class DetalleHistoricoAgendaResolveService implements Resolve<IHistoricoAgenda> {
  ...
  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<IHistoricoAgenda> {
    return this.cargaHistoricoAgendaService.getHistoricoAgenda(route.params['id']).pipe(
      catchError(error => {
